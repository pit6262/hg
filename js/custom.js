...
$(function(){
	...
	$(window).on('resize load',()=>{
		let el=$('.compare-scrollbar')[0];
		if(el.scrollWidth!=el.clientWidth){
			$('.topscroll').addClass('scroll');
			$(el).addClass('scroll')
		} else {
			$('.topscroll').removeClass('scroll');
			$(el).removeClass('scroll')
		}
	})
	var container = $('.compare-scrollbar');
	var topscroll = $('.topscroll');
	$(window).on('load scroll resize', function(){
		$('.fake').width($('.compare-table').width());
	...
	/* Fixed Column Table  */
	/* ---------------------------------------------- */
	function FixedColumnLeft() {
		$('.compare-scrollbar').on('load scroll', function(el){
			scrollLeft = el.target.scrollLeft;
			$(this).find('.th-fixed').css({
            	'transform': 'translateX(' + scrollLeft + 'px)',
            });
			if(scrollLeft > 1) {
				$(this).find('.th-fixed').addClass('active');
			} else {
				$(this).find('.th-fixed').removeClass('active');
			}
		})
